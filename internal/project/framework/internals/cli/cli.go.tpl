...
func DisplayHelpMessage(commandDescription map[string]string) {
	...
	helpMessage := `{{ .PkgName }} 1.0
	
` + chalk.Bold.TextStyle("NAME:") + `
	{{ .PkgName }} gopher - This is a CLI and make utility for {{ .PkgName }} to do awesome stuff

` + chalk.Bold.TextStyle("USAGE:") + `
	{{ .PkgName }} gopher command[:options] [arguments...]
	or
	make -- command[:options] [arguments...]

` + chalk.Bold.TextStyle("NOTES:") + `
	` + chalk.Bold.TextStyle("make") + ` supporting all ` + chalk.Bold.TextStyle("{{ .PkgName }} gopher") + ` command. Additionally, it's also supporting following:
